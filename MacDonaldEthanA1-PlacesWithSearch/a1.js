...
//registers click events and calls the respective function when clicked
window.onload=function(){
    console.log('js test')
    buildTable();
    let btnCity = document.querySelector("#btnCity");
    btnCity.addEventListener("click", buildCity)
    let btnCountry = document.querySelector("#btnCountry");
    btnCountry.addEventListener("click", buildCountry)
    let btnPop = document.querySelector("#btnPop")
    btnPop.addEventListener("click", buildPopulation)
}
// creates the table
function buildTable(){
    ...
    for(let i = 0; i < PLACES.length; i++){
        ...
        let population = Number(pieces[2]);
        htmlString += "<tr>";
        htmlString += "<td>" + city + "</td>";
        htmlString += "<td>" + country + "</td>";
        htmlString += "<td>" + population + "</td>";
        htmlString += "</tr>";
    }
    htmlString += "</table>";
    console.log(htmlString);
...
//calls buildResults() and sends values from the input(s) in
function buildCity(){
    let inputElem = document.querySelector("#city");
    let txtElem = inputElem.value.toLowerCase();
    buildResults(txtElem, 0);
    highlightTR(txtElem, 0);
...
function buildCountry(){
    let inputElem = document.querySelector("#country")
    let txtElem = inputElem.value.toLowerCase();
    buildResults(txtElem, 1);
    highlightTR(txtElem, 1);
...
function buildPopulation(){
    let inputElemMin = document.querySelector("#min")
    let inputElemMax = document.querySelector("#max")
    let numElemMin = Number(inputElemMin.value);
    let numElemMax = Number(inputElemMax.value);
    buildResults("", 2, numElemMin, numElemMax);
    highlightTR("", 2, numElemMin, numElemMax);
    document.querySelector("#country").value = '';
    document.querySelector("#city").value = '';
}
//builds the results
function buildResults(txtElem = "", arrayNum, minNum = -1, maxNum = -1){
    let htmlResults ="<ul>";
    //runs a for loop through PLACES[] and breaks it into pieces
    for(let i = 0; i < PLACES.length; i++){
        let temp  = PLACES[i];
        let pieces = temp.split(",");
        let city = pieces[0];
        let country = pieces [1];
        let population = Number(pieces[2]);
        //checks what we are searching for country city or population min and max
        if (arrayNum === 0 && city.toLowerCase().includes(txtElem)){
            htmlResults += "<li>" + PLACES[i] + "</li>";
        }
        else if(arrayNum === 1 && country.toLowerCase().includes(txtElem)){
            htmlResults += "<li>" + PLACES[i] + "</li>";
        }
        else if(arrayNum === 2 && population >= minNum && population <= maxNum){
...
//highlights every tr that matches the buildResults results
function highlightTR(txtElem = "", arrayNum, minNum = -1, maxNum = -1){
    //removes the previous highlight
    let rows = document.querySelectorAll('#table tr');
    rows.forEach(row => row.classList.remove('highlight'));
    //adds the .highlight css to all matching columns
    for(let i = 0; i < PLACES.length; i++){
        let temp = PLACES[i];
        let pieces = temp.split(",");
        let city = pieces[0].toLowerCase();
        let country = pieces[1].toLowerCase();
        let population = Number(pieces[2]);
        if(arrayNum === 0 && city.toLowerCase().includes(txtElem)){
